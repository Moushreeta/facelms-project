...
import { User, Users, View, MentorScore, DailyAttendance, PendingChange, RoleName } from '../types';
                ...
                {pending.map(change => (
                    <div key={change.changeId} className="p-3 bg-white rounded-lg shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center">
                        ...
                        <div className="flex space-x-2 self-end sm:self-center">
                            <button onClick={() => onApprove(change.changeId, true)} className="p-2 bg-green-100 text-green-600 rounded-full hover:bg-green-200"><CheckCircle size={20}/></button>
                            <button onClick={() => onApprove(change.changeId, false)} className="p-2 bg-red-100 text-red-600 rounded-full hover:bg-red-200"><XIcon size={20}/></button>
                        </div>
                    </div>
                ))}
            </div>
...
const Dashboard: React.FC<DashboardProps> = ({ currentUser, allUsers, setView, mentorScores, dailyAttendance, pendingChanges, setPendingChanges, setUsers }) => {
    ...
    const handleApproval = (changeId: string, approved: boolean) => {
        const change = pendingChanges.find(c => c.changeId === changeId);
        if(!change) return;
        if (approved) {
            setUsers(prevUsers => ({ ...prevUsers, [change.mentorId]: { ...prevUsers[change.mentorId], [change.fieldToChange]: change.newValue } }));
        }
        setPendingChanges(prev => prev.map(c => c.changeId === changeId ? { ...c, status: approved ? 'approved' : 'rejected' } : c));
    };
