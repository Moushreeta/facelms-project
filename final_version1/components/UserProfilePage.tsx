...
import { User, Users, MentorScore, DailyAttendance, View, Subjects } from '../types';
...
const UserProfilePage: React.FC<UserProfilePageProps> = ({ userId, allUsers, subjects, setView, mentorScores, dailyAttendance }) => {
                        ...
                        <h3 className="font-bold text-lg mb-4 text-gray-700">Detailed Assessment Scores</h3>
                         <div className="overflow-x-auto">
                            <table className="w-full text-sm text-left text-gray-500">
                                ...
                                <tbody>
                                    {mentorScores.filter(s => s.mentorId === userId).map(s => (<tr key={s.scoreId} className="bg-white border-b"><td className="px-4 py-4 font-medium">{subjects[s.subjectId]?.name}</td><td className="px-4 py-4">{s.type}</td><td className="px-4 py-4">{s.assessmentNumber}</td><td className="px-4 py-4 font-bold">{s.score}</td></tr>))}
                                </tbody>
                            </table>
                        </div>
                    </Card>
